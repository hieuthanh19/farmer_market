...
public class TestAddToCart extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int productID =1 ; //Bien thay doi
        double price = appDatabase.orderDetail().getProductPriceByProductID(productID);
        ...
        addTocart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (arrCart.size()!=0){
                    ...
                    arrCartByProductID = appDatabase.orderDetail().getOrderDetailByOrderIDAndProductID(productID,ordersID);
                    if (!arrCartByProductID.isEmpty()){
                        int quantity = arrCartByProductID.get(0).quantity+1;
                        productPrice = appDatabase.orderDetail().getProductPriceByProductID(productID);
                        appDatabase.orderDetail().updateQuantityAndPrice(productID,ordersID,quantity,quantity*productPrice);
                    }else{
                        //productPrice = appDatabase.orderDetail().getProductPriceByProductID(productID);
                        //appDatabase.orderDetail().insertOrderDetail(new OrderDetail(ordersID, productID, 1, productPrice, "", 1));
                    }
                }else{
